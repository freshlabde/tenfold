// tools/shots-privacy.mjs - the privacy policy as it actually stands.
//
//   PORT=7712 TENFOLD_DATA=/tmp/tenfold-shot-data node tools/serve.js &
//   BASE=http://127.0.0.1:7712 node tools/shots-privacy.mjs
//
// Writes design/screens/67-privacy.png at the reference iPhone size, from the
// production path (/privacy.html), so the picture is of the page with the CSP a
...
// than guessed: a policy is long by nature and it is worth knowing where the
// fold falls.
...
await page.screenshot({ path: "design/screens/67-privacy.png" });
console.log("wrote 67-privacy");
